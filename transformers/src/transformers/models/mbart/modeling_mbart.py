...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = _expand_mask(attention_mask, inputs_embeds.dtype, wait_k=1 if self.config.wait_k!=-1 else -1) ## Raj: Just make the mask wait-k with a k=1 and we are good to go. We want to have a unidirectional encoder no matter what.
        ## Modified by Raj Dabre. End.
